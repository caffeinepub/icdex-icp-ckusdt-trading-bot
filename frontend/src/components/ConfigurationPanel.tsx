...
export function ConfigurationPanel() {
    ...
    const validate = (): boolean => {
        ...
        if (isNaN(form.spreadPips) || form.spreadPips < 1 || form.spreadPips > 10000) {
            next.spreadPips = 'Must be 1–10000 pips';
        }
        if (isNaN(form.numOrders) || form.numOrders < 2 || form.numOrders > 50 || form.numOrders % 2 !== 0) {
            next.numOrders = 'Must be an even number between 2–50';
        }
        if (isNaN(form.intervalSeconds) || form.intervalSeconds < 5 || form.intervalSeconds > 3600) {
            next.intervalSeconds = 'Must be 5–3600 seconds';
        }
        if (isNaN(form.orderSize) || form.orderSize <= 0) {
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center justify-between">
                            <Label className="text-xs font-mono text-muted-foreground">Pip Spacing</Label>
                            <span className="text-[10px] font-mono text-muted-foreground opacity-60 uppercase tracking-widest">pips</span>
                        </div>
                        <Input
                            type="number"
                        ...
                        <span className="text-[10px] font-mono text-muted-foreground opacity-50 leading-tight">
                            ...
                            {form.spreadPips > 0 && (
                                <> Current spacing: <span className="text-foreground/70">{pipToPrice(form.spreadPips)}</span> ICP/ckBTC per level.</>
                            )}
                        </span>
                        {errors.spreadPips && (
                            <span className="text-[10px] font-mono text-terminal-sell">{errors.spreadPips}</span>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center justify-between">
                            <Label className="text-xs font-mono text-muted-foreground">Grid Orders</Label>
                            <span className="text-[10px] font-mono text-muted-foreground opacity-60 uppercase tracking-widest">orders</span>
                        </div>
                        <Input
                            type="number"
                        ...
                        {errors.numOrders && (
                            <span className="text-[10px] font-mono text-terminal-sell">{errors.numOrders}</span>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center justify-between">
                            <Label className="text-xs font-mono text-muted-foreground">Refresh Interval</Label>
                            <span className="text-[10px] font-mono text-muted-foreground opacity-60 uppercase tracking-widest">sec</span>
                        </div>
                        <Input
                            type="number"
                            ...
                            min={5}
                            max={3600}
                            onChange={(e) => handleNumberChange('intervalSeconds', e.target.value)}
                        ...
                        {errors.intervalSeconds && (
                            <span className="text-[10px] font-mono text-terminal-sell">{errors.intervalSeconds}</span>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center justify-between">
                            <Label className="text-xs font-mono text-muted-foreground">Order Size (ICP per level)</Label>
                            <span className="text-[10px] font-mono text-muted-foreground opacity-60 uppercase tracking-widest">ICP</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Input
                                type="number"
                                ...
                                min={1}
                                step={1}
                                onChange={(e) => handleNumberChange('orderSize', e.target.value)}
                                ...
                                placeholder="e.g. 5"
                            />
                            <span className="text-xs font-mono text-muted-foreground shrink-0">ICP</span>
                        </div>
                        <span className="text-[10px] font-mono text-muted-foreground opacity-50 leading-tight">
                            Fixed ICP quantity placed at each grid level. Must be greater than 0.
                            {form.orderSize > 0 && form.numOrders >= 2 && (
                                <> Total exposure: <span className="text-foreground/70">{(form.orderSize * form.numOrders).toFixed(0)} ICP</span> across all {form.numOrders} levels.</>
                            )}
                        </span>
                        {errors.orderSize && (
                            <span className="text-[10px] font-mono text-terminal-sell">{errors.orderSize}</span>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 bg-muted/10 border border-border/50 rounded px-3 py-2.5">
                        ...
                        <div className="flex flex-col gap-0.5">
                            ...
                            <div className="grid grid-cols-[5rem_1fr_auto] gap-2 pb-1 border-b border-border/30 mb-0.5">
                                <span className="text-[9px] font-mono text-muted-foreground opacity-50 uppercase">Side</span>
                                <span className="text-[9px] font-mono text-muted-foreground opacity-50 uppercase text-right">Price</span>
                                <span className="text-[9px] font-mono text-muted-foreground opacity-50 uppercase text-right">Size</span>
                            </div>
                            {Array.from({ length: halfOrders > 0 ? Math.min(halfOrders, 4) : 0 })
                                .map((_, i) => {
                                    const level = Math.min(halfOrders, 4) - i;
                                    const price = exampleMid + level * pipSpacing;
                                    return (
                                        <div key={`sell-prev-${i}`} className="grid grid-cols-[5rem_1fr_auto] gap-2 text-[10px] font-mono">
                                            <span className="text-terminal-sell">SELL +{level}×pip</span>
                                            <span className="text-terminal-sell tabular-nums text-right">{price.toFixed(8)}</span>
                                            <span className="text-terminal-sell tabular-nums text-right opacity-80">{form.orderSize > 0 ? form.orderSize : '—'}</span>
                                        </div>
                                    );
                                })
                                .reverse()}
                            <div className="grid grid-cols-[5rem_1fr_auto] gap-2 text-[10px] font-mono border-t border-b border-border/40 py-0.5 my-0.5">
                                ...
                                <span className="text-foreground/80 tabular-nums text-right">{exampleMid.toFixed(8)}</span>
                                <span className="text-muted-foreground opacity-40 text-right">—</span>
                            </div>
                            {Array.from({ length: halfOrders > 0 ? Math.min(halfOrders, 4) : 0 })
                                .map((_, i) => {
                                    const level = i + 1;
                                    const price = exampleMid - level * pipSpacing;
                                    return (
                                        <div key={`buy-prev-${i}`} className="grid grid-cols-[5rem_1fr_auto] gap-2 text-[10px] font-mono">
                                            <span className="text-terminal-buy">BUY  −{level}×pip</span>
                                            <span className="text-terminal-buy tabular-nums text-right">{price.toFixed(8)}</span>
                                            <span className="text-terminal-buy tabular-nums text-right opacity-80">{form.orderSize > 0 ? form.orderSize : '—'}</span>
                                        </div>
                                    );
                                })}
                        </div>
            ...
            <div className="flex flex-col gap-1 text-[10px] font-mono text-muted-foreground bg-muted/20 border border-border/60 rounded px-3 py-2">
                ...
                {config && (
                    <div className="flex items-center gap-1.5 border-t border-border/40 pt-1 mt-0.5">
                        ...
                        <span className="text-terminal-buy font-semibold">{Number(config.orderSize)} ICP</span>
