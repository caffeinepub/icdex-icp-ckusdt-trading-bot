...
import { useCancelAllOrders, useOpenOrders } from "@/hooks/useQueries";
import {
  AlertCircle,
  CheckCircle2,
  Inbox,
...
function OrderRow({
  ...
  index,
  dimmed,
}: { entry: OrderEntry; index: number; dimmed?: boolean }) {
  const isBuy = entry.side === Side.buy;
  return (
    <div
      className={`grid grid-cols-[2.5rem_3rem_1fr_1fr_3.5rem] gap-2 px-3 py-2 border-b border-border/40 hover:bg-muted/20 transition-colors ${dimmed ? "opacity-30" : ""}`}
    >
      <span className="text-[10px] font-mono text-muted-foreground self-center">
        #{index + 1}
      </span>
      <span
        className={`text-[10px] font-mono font-semibold self-center ${isBuy ? "text-terminal-buy" : "text-terminal-sell"}`}
      >
        {isBuy ? "BUY" : "SELL"}
      </span>
      <span
        className={`text-xs font-mono text-right self-center tabular-nums ${isBuy ? "text-terminal-buy" : "text-terminal-sell"}`}
      >
        ...
        <div className="relative">
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-card/70 rounded">
            ...
            <span className="text-xs font-mono text-terminal-warning tracking-widest uppercase">
              Cancelling {allOrders.length} order
