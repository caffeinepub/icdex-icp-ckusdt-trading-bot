...
export function GridPreviewTable() {
  ...
  const lowestSell =
    backendSells.length > 0 ? backendSells[backendSells.length - 1] : null;
  const highestBuy = backendBuys.length > 0 ? backendBuys[0] : null;
  const midPriceFromGrid =
    lowestSell && highestBuy ? (lowestSell + highestBuy) / BigInt(2) : null;
  ...
  const useSymmetricPreview =
    midPriceFromGrid !== null && midPriceFromGrid > BigInt(0);
  ...
  const normalisedOpenOrders: Array<{ side: string; price: bigint }> = (
    openOrders ?? []
  ).map((o) => ({
  ...
  const placedSells = displaySells.filter((p) =>
    hasOpenOrderAtLevel(normalisedOpenOrders, "sell", p),
  ).length;
  const placedBuys = displayBuys.filter((p) =>
    hasOpenOrderAtLevel(normalisedOpenOrders, "buy", p),
  ).length;
        ...
        <div className="flex items-center gap-2">
          {hasData && !isInitialLoading && totalLevels > 0 && (
            <span className="text-[10px] font-mono text-muted-foreground">
              <span className="text-terminal-buy">{totalPlaced}</span>/
              {totalLevels} placed
            </span>
          )}
          <button
      ...
      {isInitialLoading ? (
        ...
        <div className="flex flex-col gap-2">
          {["a", "b", "c", "d", "e", "f", "g", "h"].map((id) => (
            <Skeleton key={`skeleton-${id}`} className="h-7 w-full bg-muted" />
          ))}
        </div>
      ) : isError ? (
        /* Error state */
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs font-mono text-muted-foreground opacity-60">
              {isRunning
                ...
                : "Start the bot to generate grid levels"}
            </span>
          </div>
          {isFetching && (
        ...
        <ScrollArea className="flex-1 max-h-[380px]">
          ...
          <div className="grid grid-cols-[3rem_1fr_5rem_4rem_2rem] gap-2 px-3 py-1.5 border-b border-border sticky top-0 bg-card z-10">
            ...
            <span className="text-[10px] font-mono text-muted-foreground text-right opacity-60">
              LVL
            </span>
          </div>
          ...
          {displaySells.map((price, i) => {
            ...
            const isPlaced = hasOpenOrderAtLevel(
              normalisedOpenOrders,
              "sell",
              price,
            );
            return (
        ...
        <div className="flex items-center text-xs font-mono text-muted-foreground pt-1 border-t border-border">
          ...
          {hasData && (
            <span className="ml-auto opacity-60">
              {displaySells.length + displayBuys.length} levels · {pipSpacing}{" "}
              pips · {orderSize} ICP/lvl
            </span>
          )}
        </div>
      )}
