...
function LogRow({ entry }: { entry: LogEntry }) {
  ...
  return (
    <div
      className={`flex items-start gap-3 py-1.5 border-b border-border/30 last:border-0 ${isOrderError ? "bg-destructive/5" : ""}`}
    >
      ...
      <div className="flex items-start gap-1.5 min-w-0">
        ...
        <span
          className={`text-xs font-mono leading-relaxed break-all ${eventColor(entry.eventType, entry.message)}`}
        >
          {entry.message}
        </span>
      </div>
